...
extension ESI {
	...
	public struct Industry {
		...
		public struct Facilities {
			...
			public func get(cachePolicy: URLRequest.CachePolicy = .useProtocolCachePolicy, progress: Request.ProgressHandler? = nil) -> AnyPublisher<ESIResponse<[ESI.Industry.Facilities.Success]>, AFError> {
				do {
					...
					return Deferred { () -> AnyPublisher<ESIResponse<[ESI.Industry.Facilities.Success]>, AFError> in
		...
		public struct Systems {
			...
			public func get(cachePolicy: URLRequest.CachePolicy = .useProtocolCachePolicy, progress: Request.ProgressHandler? = nil) -> AnyPublisher<ESIResponse<[ESI.Industry.Systems.Success]>, AFError> {
				do {
					...
					return Deferred { () -> AnyPublisher<ESIResponse<[ESI.Industry.Systems.Success]>, AFError> in
						var request = session.request(components, method: .get, encoding: URLEncoding.default, headers: headers, interceptor: CachePolicyAdapter(cachePolicy: cachePolicy))
